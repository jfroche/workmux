...
| `--pr <number\|url>`           | Checkout a pull/merge request by its number or full URL into a new worktree. On GitHub repositories this uses the `gh` CLI (`https://github.com/<owner>/<repository>/pull/<number>`); on GitLab repositories (origin host `gitlab.com` or a `gitlab.*` host) it uses the `glab` CLI to fetch the merge request. The forge is detected automatically from the `origin` remote. The corresponding CLI must be installed and authenticated. The local branch name defaults to the PR/MR's head/source branch name, but can be overridden (e.g., `workmux add custom-name --pr 123`). |
